...
sphere_radius_val = ti.field(dtype=float, shape=1)
...
def init_springs():
    ...
    for i, j in ti.ndrange(N, N):
        ...
        if i < N - 1 and j > 0:
            ...
            spring_pairs[c] = ti.Vector([idx, idx_diag])
            spring_lengths[c] = (x[idx] - x[idx_diag]).norm()
        # 隔一个点的连接 (弯曲弹簧 - Bending)
        if i < N - 2:
            idx_bend_x = (i + 2) * N + j
            c = ti.atomic_add(num_springs[None], 1)
            spring_pairs[c] = ti.Vector([idx, idx_bend_x])
            spring_lengths[c] = (x[idx] - x[idx_bend_x]).norm()
        if j < N - 2:
            idx_bend_y = i * N + (j + 2)
            c = ti.atomic_add(num_springs[None], 1)
            spring_pairs[c] = ti.Vector([idx, idx_bend_y])
            spring_lengths[c] = (x[idx] - x[idx_bend_y]).norm()
...
@ti.func
def sphere_collision(pos: ti.template(), vel: ti.template(), idx: int):
    """球体碰撞检测与响应"""
    center = sphere_center[0]
    radius = sphere_radius_val[0]
    diff = pos[idx] - center
    dist = diff.norm()
    if dist < radius:
        normal = diff / dist
        pos[idx] = center + normal * radius
        vel[idx] = vel[idx] - ti.Vector.dot(vel[idx], normal) * normal
    ...
    for _ in ti.static(range(3)):
        ...
        for i in range(N * N):
            if is_fixed[i] == 0:
                ...
                sphere_collision(x_next, v_next, i)
    # 3. 将收敛后的状态写回
    for i in range(N * N):
        v[i] = v_next[i]
        x[i] = x_next[i]
...
def main():
    ...
    while window.running:
        ...
        # 绘制球体 (用大粒子模拟)
        scene.particles(sphere_center, radius=sphere_radius_val[0], color=(0.8, 0.4, 0.2))
